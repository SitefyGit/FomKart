import React from 'react'
import { TrendingUp, Cookie, Flag, Shield, ExternalLink } from 'lucide-react'
import LeadCaptureForm from './LeadCaptureForm'

interface GrowthHackFooterProps {
  className?: string
  creatorId?: string
  showNewsletterForm?: boolean
  onGrowthHack?: () => void
  onCookiePreferences?: () => void
  onReport?: () => void
  onPrivacyPolicy?: () => void
}

export function GrowthHackFooter({ 
  className = "",
  creatorId,
  showNewsletterForm = false,
  onGrowthHack,
  onCookiePreferences,
  onReport,
  onPrivacyPolicy 
}: GrowthHackFooterProps) {
  const handleGrowthHack = () => {
    if (onGrowthHack) {
      onGrowthHack()
    } else {
      // Default growth hack functionality
      alert('Growth Hack features coming soon! 🚀\n\nThis will include:\n• Analytics dashboard\n• Audience insights\n• Growth recommendations\n• Performance metrics')
    }
  }

  const handleCookiePreferences = () => {
    if (onCookiePreferences) {
      onCookiePreferences()
    } else {
      // Default cookie preferences
      alert('Cookie Preferences\n\nManage your cookie settings and privacy preferences here.')
    }
  }

  const handleReport = () => {
    if (onReport) {
      onReport()
    } else {
      // Default report functionality
  alert('Report Content\n\nHelp us keep FomKart safe by reporting inappropriate content or behavior.')
    }
  }

  const handlePrivacyPolicy = () => {
    if (onPrivacyPolicy) {
      onPrivacyPolicy()
    } else {
      // Default privacy policy
      window.open('/privacy-policy', '_blank')
    }
  }

  return (
    <footer className={`bg-gray-900 text-white py-8 mt-16 ${className}`}>
      <div className="max-w-4xl mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 md:gap-6">
            <button 
              onClick={handleGrowthHack}
              className="flex items-center space-x-2 text-emerald-400 hover:text-emerald-300 transition-colors group"
            >
              <TrendingUp className="h-4 w-4 group-hover:scale-110 transition-transform" />
              <span className="font-medium">Growth Hack</span>
            </button>
            
            <button 
              onClick={handleCookiePreferences}
              className="flex items-center space-x-2 hover:text-gray-300 transition-colors group"
            >
              <Cookie className="h-4 w-4 group-hover:scale-110 transition-transform" />
              <span>Cookie Preferences</span>
            </button>
            
            <button 
              onClick={handleReport}
              className="flex items-center space-x-2 hover:text-gray-300 transition-colors group"
            >
              <Flag className="h-4 w-4 group-hover:scale-110 transition-transform" />
              <span>Report</span>
            </button>
            
            <button 
              onClick={handlePrivacyPolicy}
              className="flex items-center space-x-2 hover:text-gray-300 transition-colors group"
            >
              <Shield className="h-4 w-4 group-hover:scale-110 transition-transform" />
              <span>Privacy Policy</span>
            </button>
          </div>
          
          <div className="text-center md:text-right">
            <div className="text-sm text-gray-400">
              © {new Date().getFullYear()} FomKart. All rights reserved.
            </div>
            <div className="text-xs text-gray-500 mt-1">
              Empowering creators worldwide
            </div>
          </div>
        </div>

        {/* Additional Links Row */}
        <div className="mt-8 pt-6 border-t border-gray-800">
          {/* Newsletter Form Section - Above Social Media Links */}
          {showNewsletterForm && creatorId && (
            <div className="mb-8">
              <div className="max-w-md mx-auto">
                <LeadCaptureForm
                  creatorId={creatorId}
                  compactMode={true}
                  title="📧 Join Our Newsletter"
                  subtitle="Get exclusive updates and offers"
                  placeholder="Your email address"
                  buttonText="Subscribe"
                  showNameField={false}
                  showPreferences={false}
                  className="bg-gray-800 border-gray-700"
                  onSubscribe={(data) => {
                    console.log('Footer newsletter subscription:', data)
                    // Show success notification
                    const notification = document.createElement('div')
                    notification.className = 'fixed top-4 right-4 bg-emerald-500 text-white px-4 py-2 rounded-lg shadow-lg z-50'
                    notification.textContent = 'Thanks for subscribing! 🎉'
                    document.body.appendChild(notification)
                    setTimeout(() => notification.remove(), 3000)
                  }}
                />
              </div>
            </div>
          )}
          
          <div className="flex flex-wrap justify-center md:justify-between items-center gap-4">
            <div className="flex flex-wrap gap-4 text-sm text-gray-400">
              <button className="hover:text-white transition-colors">Terms of Service</button>
              <button className="hover:text-white transition-colors">Help Center</button>
              <button className="hover:text-white transition-colors">Contact Support</button>
              <button className="hover:text-white transition-colors">Community Guidelines</button>
            </div>
            
            <div className="flex items-center space-x-4">
              <a 
                href="https://twitter.com/fomkart" 
                target="_blank" 
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-white transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                </svg>
              </a>
              
              <a 
                href="https://instagram.com/fomkart" 
                target="_blank" 
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-white transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
              
              <a 
                href="https://linkedin.com/company/fomkart" 
                target="_blank" 
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-white transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
              </a>
            </div>
          </div>
        </div>
      </div>
    </footer>
  )
}
