    ...
    <section className="py-12 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-12">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4">Popular services</h2>
          <p className="text-base sm:text-lg text-gray-600">Trending services from our community</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {servicesWithRatings.map((service: Service, index: number) => (
            <Link
              ...
              prefetch
              className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden transform hover:scale-[1.03] hover:-translate-y-2"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="aspect-video bg-gradient-to-br from-emerald-100 via-blue-50 to-purple-100 flex items-center justify-center text-3xl sm:text-4xl group-hover:scale-110 transition-transform duration-300 relative overflow-hidden">
              ...
              <div className="p-4 sm:p-6">
                <h3 className="font-semibold text-gray-900 mb-3 line-clamp-2 group-hover:text-emerald-600 transition-colors text-sm sm:text-base">{service.title}</h3>
                ...
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <span className="text-xs sm:text-sm text-gray-600 group-hover:text-gray-700 transition-colors truncate">{service.creator?.full_name || service.creator?.username}</span>
                </div>
