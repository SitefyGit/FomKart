        ...
        <div className="fixed inset-0 bg-black/20 backdrop-blur-[1.5px] flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200 rounded-2xl shadow-lg w-full max-w-md p-6 space-y-5">
            ...
            <div className="flex flex-col items-center text-center -mt-8">
              ...
              <form onSubmit={async e=>{e.preventDefault(); if(!subscribeEmail) return; setSubscribing(true); try { const res = await fetch('/api/newsletter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: subscribeEmail, creatorId: creator.id, source: 'creator_profile' }) }); if (!res.ok) throw new Error('Subscribe failed'); setSubscribeEmail(''); setSubscribeOpen(false); } catch(err){ console.warn('Subscribe failed', err);} finally { setSubscribing(false);} }} className="w-full flex rounded-lg overflow-hidden border dark:border-gray-600 focus-within:ring-2 focus-within:ring-blue-500">
