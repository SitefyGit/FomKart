      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-xl shadow border border-gray-100 p-6">
              ...
              <div className="mt-5 flex gap-2 border-b">
                {(['activity','details','requirements','delivery'] as TabKey[]).map(t => (
                  <button key={t} onClick={()=>setActiveTab(t)} className={`px-3 py-2 text-sm font-medium -mb-px border-b-2 ${activeTab===t ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-600 hover:text-gray-800'}`}>{t[0].toUpperCase()+t.slice(1)}</button>
                ))}
              </div>
              ...
              <div className="mt-6">
                {activeTab === 'activity' && (
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3 text-sm text-gray-600">
                      <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-600"/> Order placed</div>
                      {order.requirements ? <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-600"/> Requirements submitted</div> : null}
                      {order.status==='in_progress' && <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-blue-600"/> Order in progress</div>}
                      {order.status==='delivered' && <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-600"/> Delivered</div>}
                      {order.status==='completed' && <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-600"/> Completed</div>}
                    </div>
                    {/* Messages */}
                    <div className="space-y-3">
                      {messages.map((m:any)=> {
                        ...
                        if (typeof m.message === 'string') {
                          ...
                          if (text.includes('approved')) { accent='bg-green-600'; bg='bg-green-50'; border='border-green-100'; Icon=BadgeCheck }
                          else if (text.includes('revision')) { accent='bg-amber-600'; bg='bg-amber-50'; border='border-amber-100'; Icon=RefreshCcw }
                          else if (text.includes('delivery')) { accent='bg-indigo-600'; bg='bg-indigo-50'; border='border-indigo-100'; Icon=PackageOpen }
                        }
                        if (isSys) {
                          return (
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center justify-between mb-1">
                                    ...
                                    <div className="inline-flex items-center gap-1 text-[11px] font-medium text-gray-700">
                                      <Icon className="w-3.5 h-3.5"/> System
                                    </div>
                                  </div>
                                  <div className="text-sm whitespace-pre-wrap text-gray-900">{m.message}</div>
                                  {Array.isArray(m.attachments) && m.attachments.length>0 && (
                                    <div className="mt-2 flex flex-wrap gap-2">
                                      {m.attachments.map((a:string, i:number)=> (
                                        <a key={i} href={a} target="_blank" className="text-xs text-blue-600 underline flex items-center gap-1"><Paperclip className="w-3 h-3"/> Attachment {i+1}</a>
                                      ))}
                                    </div>
                                  )}
                                </div>
                        ...
                        return (
                          <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] flex items-start gap-2 ${mine ? 'flex-row-reverse' : ''}`}>
                              {/* Avatar */}
                              <div className="w-8 h-8 rounded-full bg-gray-200 overflow-hidden flex-shrink-0 flex items-center justify-center text-[10px] text-gray-600">
                                {avatar ? (
                              ...
                              {/* Bubble */}
                              <div className={`p-3 rounded-xl border ${mine ? 'bg-blue-50 border-blue-100' : 'bg-gray-50 border-gray-100'}`}>
                                <div className={`text-[11px] mb-1 ${mine ? 'text-blue-700' : 'text-gray-700'}`}>
                    ...
                    <div className="flex gap-2 mt-3 items-center">
                      ...
                      <input value={compose} onChange={e=>setCompose(e.target.value)} className="flex-1 border rounded-lg px-3 py-2 text-sm" placeholder="Write a message" />
                      <button onClick={handleSend} disabled={posting || uploading || (!compose.trim() && !(attachments && attachments.length))} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm disabled:opacity-50 flex items-center gap-1">
                        <Send className="w-4 h-4"/>
                        {posting || uploading ? 'Sending…' : 'Send'}
                      </button>
                    </div>
                    {attachments && attachments.length > 0 && (
                      <div className="text-xs text-gray-500 mt-1">{attachments.length} file(s) attached</div>
                    )}
                  </div>
                )}
                ...
                {activeTab === 'details' && (
                  <div className="space-y-6">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="font-semibold text-gray-900 text-lg">{order.product?.title}</div>
                        <div className="text-sm text-gray-600">by {order.seller?.full_name || order.seller?.username}</div>
                      </div>
                      <div className="inline-flex items-center gap-2 text-xs">
                        <span className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-700">{order.package?.name || 'Standard'} package</span>
                        {order.package?.delivery_time ? (
                          <span className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">{order.package.delivery_time} days</span>
                        ) : null}
                      </div>
                    </div>
                        ...
                        <tbody>
                          <tr className="border-t">
                            <td className="p-4 align-top">
                              <div className="font-medium">{order.package?.name || 'Standard'}</div>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex justify-between"><span className="text-gray-600">Service Fee</span><span className="font-medium">${fee.toFixed(2)}</span></div>
                          <div className="flex justify-between text-base font-semibold pt-2 border-t"><span>Total</span><span>${total.toFixed(2)}</span></div>
                        </div>
                      )
                    })()}
                  </div>
                ...
                {activeTab === 'requirements' && (
                  <div className="space-y-4">
                    {!order.requirements ? (
                      <div className="space-y-3">
                        <div className="p-4 rounded-lg border border-amber-200 bg-amber-50 text-amber-800 text-sm">No requirements submitted yet.</div>
                        {isBuyer && (
                        ...
                        {!isBuyer && (
                          <div className="text-sm text-gray-600">Waiting for buyer to submit requirements.</div>
                        )}
                      </div>
                    ) : (
                      <div className="space-y-2 text-sm">
                        {Object.entries(order.requirements).map(([k,v]:any)=> (
                          <div key={k} className="p-3 border border-gray-100 rounded-xl bg-white">
                            <div className="text-gray-500 text-xs uppercase">{k}</div>
                            <div className="text-gray-900 whitespace-pre-wrap">{String(v)}</div>
                          </div>
                        ))}
                        {isSeller && (
                ...
                {activeTab === 'delivery' && (
                  <div className="space-y-4">
                    {isBuyer && order.status==='delivered' && (
                      <div className={`p-4 rounded-lg border ${overdue ? 'bg-red-50 border-red-200 text-red-700' : 'bg-yellow-50 border-yellow-200 text-yellow-800'}`}>
                        {overdue ? 'Approval window ended. Please review the delivery.' : `Please review the delivery by ${approveByLabel || 'the deadline'}.`}
                      </div>
                    )}
                    {deliveries.length===0 && <div className="p-4 rounded-lg border border-gray-100 bg-gray-50 text-gray-600 text-sm">No deliveries yet.</div>}
                    {deliveries.map((d:any, idx:number)=> (
                      <div key={d.id} className="border border-gray-100 rounded-xl p-4 bg-white">
                        <div className="text-sm text-gray-500 mb-2">Delivery #{idx+1} • {new Date(d.created_at).toLocaleString()}</div>
                        {d.description && <div className="text-sm text-gray-900 whitespace-pre-wrap">{d.description}</div>}
                        <div className="mt-2 flex flex-wrap gap-2">
                          <a href={d.file_url} target="_blank" className="text-xs text-blue-600 underline flex items-center gap-1"><Paperclip className="w-3 h-3"/> {d.file_name}</a>
                        </div>
                      </div>
                    ))}
                    {isBuyer && order.status==='delivered' && (
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button onClick={requestRevision} className="px-4 py-2 border rounded-lg text-sm hover:bg-gray-50">Request revision</button>
                      </div>
                    )}
          ...
          <div className="space-y-6">
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between"><span className="text-gray-600">Package Price</span><span className="font-medium">${subtotal.toFixed(2)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600">Service Fee (5%)</span><span className="font-medium">${fee.toFixed(2)}</span></div>
                    <div className="border-t pt-3 flex justify-between font-semibold text-lg"><span>Total</span><span>${total.toFixed(2)}</span></div>
                  </div>
                )
              })()}
              <button onClick={()=>setShowBilling(v=>!v)} className="mt-4 text-sm text-blue-600 hover:underline inline-flex items-center gap-1">
            ...
            <div className="bg-white rounded-xl shadow border border-gray-100 p-6">
              <h3 className="font-semibold text-gray-900 mb-3">Track Order</h3>
              <ol className="space-y-2 text-sm">
                <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-600"/> Order placed</li>
                <li className="flex items-center gap-2">{order.requirements ? <CheckCircle className="w-4 h-4 text-green-600"/> : <Clock className="w-4 h-4 text-gray-400"/>} Requirements submitted</li>
                <li className="flex items-center gap-2">{order.status==='in_progress' || order.status==='delivered' || order.status==='completed' ? <CheckCircle className="w-4 h-4 text-green-600"/> : <Clock className="w-4 h-4 text-gray-400"/>} Order in progress</li>
                <li className="flex items-center gap-2">{order.status==='delivered' || order.status==='completed' ? <CheckCircle className="w-4 h-4 text-green-600"/> : <Clock className="w-4 h-4 text-gray-400"/>} Review delivery</li>
                <li className="flex items-center gap-2">{order.status==='completed' ? <CheckCircle className="w-4 h-4 text-green-600"/> : <Clock className="w-4 h-4 text-gray-400"/>} Complete order</li>
              </ol>
              <div className="mt-3 text-xs text-gray-500">Expected delivery: {expectedDate || (order.package?.delivery_time ? `within ${order.package.delivery_time} days` : '—')}</div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2"><PackageOpen className="w-5 h-5"/><h3 className="font-semibold">Send Delivery</h3></div>
              <button onClick={()=>setShowDeliverModal(false)} className="text-gray-500">✕</button>
            </div>
            <div className="space-y-3">
              <textarea value={deliveryNote} onChange={e=>setDeliveryNote(e.target.value)} className="w-full border rounded-lg p-2 text-sm" rows={4} placeholder="Add a note about this delivery (optional)" />
              ...
              {deliveryFiles && deliveryFiles.length>0 && (
                <div className="text-xs text-gray-500">{deliveryFiles.length} file(s) selected</div>
              )}
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={()=>setShowDeliverModal(false)} className="px-3 py-2 border rounded-lg text-sm">Cancel</button>
              <button onClick={handleSendDelivery} disabled={uploading || !(deliveryFiles && deliveryFiles.length)} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm disabled:opacity-50">Send delivery</button>
            </div>
          </div>
        </div>
      )}
